    ...
    async fetch(request, env, ctx): Promise<Response> {
        try {
            ...
            if (!isWebSocket) {
                ...
                if (
                    authorizationValue !==
                        `Bearer ${env.ADMIN_AUTHORIZATION_TOKEN}` &&
                    authorizationValue !==
                        `Bearer ${env.CLIENT_AUTHORIZATION_TOKEN}`
                ) {
                    const authorizationWithoutBearer =
                    ...
                    if (env.AUTH_JWKS_ENDPOINT && env?.AUTH_ALGORITHM) {
                        try {
                            const JWKS = createRemoteJWKSet(
                                new URL(env.AUTH_JWKS_ENDPOINT)
                            )
            ...
            const config = {
                ...
                externalDbCloudflareAccountId:
                    env.EXTERNAL_DB_CLOUDFLARE_ACCOUNT_ID,
                externalDbCloudflareDatabaseId:
