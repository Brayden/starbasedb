...
export class LiteREST {
    ...
    constructor(dataSource: DataSource, config: HandlerConfig) {
    ...
    private async getPrimaryKeyColumns(
        ...
        const isSQLite =
            this.dataSource.source === Source.internal ||
            this.config.externalDbType?.toLowerCase() === 'sqlite'
        const schemaInfo = (await executeQuery(
            query,
            this.dataSource.source === Source.external ? {} : [],
            false,
            this.dataSource,
            this.config
        )) as any[]
