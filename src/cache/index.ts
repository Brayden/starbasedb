...
export async function beforeQueryCache(
    sql: string,
    params?: any[],
    ...
    if (
        dataSource?.source === Source.internal ||
        !dataSource?.request.headers.has('X-Starbase-Cache')
    )
        return null
...
export async function afterQueryCache(
    sql: string,
